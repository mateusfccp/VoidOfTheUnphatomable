            ...
            if (playerEntity.associatedObject().hasItemOfType(ResoundingCore.class)) {
                Engine.context().sceneManager().push(
                    ...
                    if (scene instanceof InGame gameScene) {
                        final HighscoreEntry highscoreEntry = HighscoreEntry.fromGame(gameScene, RunStatus.VICTORIOUS);
                ...
                Engine.context().sceneManager().<Boolean>push(questionScene).thenAccept((result) -> {
                    if (result) {
                        Engine.context().sceneManager().push(
                            ...
                            final Scene scene = Engine.context().sceneManager().currentScene();
                            if (scene instanceof InGame gameScene) {
                                final HighscoreEntry highscoreEntry = HighscoreEntry.fromGame(gameScene, RunStatus.COWARD);
                                Engine.context().sceneManager().pop(highscoreEntry);
                            }
                        });
                    }
